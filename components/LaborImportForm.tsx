...
import { importCatalogLaborBatchAction } from "@/lib/actions";
import { decodeImportBytes, parseQuickbooksExport } from "@/lib/quickbooks";
import { pickLaborItems } from "@/lib/labor-import";

const BATCH_SIZE = 250;

export function LaborImportForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!file) {
      setError("Choose a CSV or QuickBooks file to import.");
      return;
    }
    ...
    try {
      ...
      const items = pickLaborItems(parseQuickbooksExport(text));
      if (items.length === 0) {
        const preview = text.split(/\r?\n/).slice(0, 3).join(" | ").slice(0, 180);
        setError(
          `No labor items found. Use a CSV with Name and Rate, or a QuickBooks export with Service items. First rows: ${preview || "(empty)"}`,
        );
        return;
      }
      ...
      for (let i = 0; i < items.length; i += BATCH_SIZE) {
        ...
        setStatus(`Saving ${Math.min(i + BATCH_SIZE, items.length).toLocaleString()} of ${items.length.toLocaleString()} labor items…`);
        const result = await importCatalogLaborBatchAction(batch);
        if ("error" in result) {
          setError(
      ...
      router.push(`/labor?imported=${created}&updated=${updated}`);
      router.refresh();
    } catch (err) {
      console.error(err);
