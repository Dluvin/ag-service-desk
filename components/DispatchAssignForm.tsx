...
import { TICKET_STATUSES, STATUS_LABELS, isFinishedStatus } from "@/lib/roles";
...
export function DispatchAssignForm({
  ...
  return (
    <ActionForm action={assignTicketAction} className="mt-2 space-y-2">
      ...
      <button className="w-full rounded-md bg-emerald-800 px-2 py-1 text-xs font-semibold text-white">Update</button>
    </ActionForm>
  );
}
