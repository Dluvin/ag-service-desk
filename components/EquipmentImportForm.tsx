...
import { importCatalogEquipmentBatchAction } from "@/lib/actions";
import { decodeImportBytes, parseQuickbooksExport } from "@/lib/quickbooks";
import { pickEquipmentItems } from "@/lib/equipment-import";
...
export function EquipmentImportForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const items = pickEquipmentItems(parseQuickbooksExport(text));
      if (items.length === 0) {
        const preview = text.split(/\r?\n/).slice(0, 3).join(" | ").slice(0, 180);
        setError(
          `No equipment items found. Use a CSV with Name and Rate. First rows: ${preview || "(empty)"}`,
        );
        return;
      }
      ...
      for (let i = 0; i < items.length; i += BATCH_SIZE) {
        ...
        setStatus(
          `Saving ${Math.min(i + BATCH_SIZE, items.length).toLocaleString()} of ${items.length.toLocaleString()} equipment items…`,
        );
        const result = await importCatalogEquipmentBatchAction(batch);
        if ("error" in result) {
          setError(
      ...
      router.push(`/equipment?imported=${created}&updated=${updated}`);
      router.refresh();
    } catch (err) {
      console.error(err);
  ...
  return (
    <form onSubmit={onSubmit} className="mt-3 space-y-3 rounded-xl border border-stone-200 bg-white p-4">
      ...
      >
        {busy ? "Importing…" : "Import equipment"}
      </button>
    </form>
  );
}
