...
import { AuthBrandProvider, AuthScreenLogos } from "@/components/AuthBrand";
import { ActionForm } from "@/components/ActionForm";
import { resolveOrgBrandById } from "@/lib/org-brand";
...
export default async function WelcomePage({
  ...
  const dealer = row ? await resolveOrgBrandById(row.user.organizationId) : null;
            ...
            {!row ? (
              ...
              <>
                <p className="mt-1 text-sm text-stone-600">
                  Welcome to {row.user.organization.name}. Choose a password for {row.user.email}, then
